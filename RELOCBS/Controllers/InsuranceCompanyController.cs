...
namespace RELOCBS.Controllers
{
    ...
    public class InsuranceCompanyController : BaseController
    {
        ...
        private InsuranceCompanyBL _insCompBL;
        public InsuranceCompanyBL insCompBL
        {
            get
            {
                if (this._insCompBL == null)
                    this._insCompBL = new InsuranceCompanyBL();
                return this._insCompBL;
        ...
        public ActionResult Index(int? page)
        {
            ...
            session.Set<string>("PageSession", "Insurance Company Master");
            ...
            //}
            var items = insCompBL.GetInsCompanyList(pageIndex, pageSize, OrderBy, Order, InsCompID, null, SearchKey, out totalCount);
            if (totalCount == 0 && pageIndex > 1)
            {
                pageIndex = 1;
                items = insCompBL.GetInsCompanyList(pageIndex, pageSize, OrderBy, Order, InsCompID, null, SearchKey, out totalCount);
            }

            var itemsAsIPagedList = new StaticPagedList<InsuranceCompany>(items, pageIndex, pageSize, totalCount);
        ...
        public ActionResult Create(InsuranceCompany model)
        {
            try
            {
                ...
                {
                    string Message;
                    result.Success = insCompBL.Insert(model, out Message);
                    if (result.Success)
                    {
        ...
        public ActionResult Edit(int id)
        {
            ...
            InsuranceCompany data = insCompBL.GetDetailById(id);
            if (data == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        public ActionResult Edit(int id, InsuranceCompany model)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    result.Success = insCompBL.Update(model, out message);
        ...
        {
            var lst = comboBL.GetInsuranceCompanyDropdown().Select(i => new { i.Value, i.Text }).ToList();
        ...
        public ActionResult ExportToExcel()
        {
            ...
            try
            {
                ...
                CommonService.GenerateExcel(this.Response, "Company", "[Comm].[GETInsuranceCompanyForGrid_ExpToExl]", param);

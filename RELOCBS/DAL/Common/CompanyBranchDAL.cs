...
namespace RELOCBS.DAL.Common
{
    public class CompanyBranchDAL
    {
        ...
        public bool Insert(CompanyBranchViewModel company, out string result)
        {
            ...
            try
            {
                ...
                using (App_Code.CDALSQL conn = new App_Code.CDALSQL(System.Configuration.ConfigurationManager.ConnectionStrings["RELODB"].ToString()))
                {
                    if (conn.Connect())
                    {
                        conn.AddCommand("[Comm].[AddEditCompanyBranch]", QueryType.Procedure);
                        //conn.AddParameter(ParameterOF.PROCEDURE, "@SP_CompBranchID", SqlDbType.Int, 0, ParameterDirection.InputOutput, company.CompBranchID);
                        conn.AddParameter(ParameterOF.PROCEDURE, "@SP_CompBranchName", SqlDbType.VarChar, 100, ParameterDirection.Input, CSubs.PSafeValue(company.CompBranchName));
                        conn.AddParameter(ParameterOF.PROCEDURE, "@sp_ContactPerson", SqlDbType.VarChar, 100, ParameterDirection.Input, CSubs.PSafeValue(company.ContactPerson));
                        conn.AddParameter(ParameterOF.PROCEDURE, "@sp_ContactNo", SqlDbType.VarChar, 20, ParameterDirection.Input, CSubs.PSafeValue(company.ContactNo));
                        conn.AddParameter(ParameterOF.PROCEDURE, "@SP_CompId", SqlDbType.Int, 0, ParameterDirection.Input, company.CompId);
                        conn.AddParameter(ParameterOF.PROCEDURE, "@SP_CityID", SqlDbType.Int, 0, ParameterDirection.Input, company.CityID);
                        conn.AddParameter(ParameterOF.PROCEDURE, "@SP_IsRevenueBr", SqlDbType.Bit, 0, ParameterDirection.Input, company.IsRevenueBr);
            ...
            {
                throw new DataAccessException(Convert.ToString(UserSession.GetUserSession().LoginID), "CompanyBranchDAL", "Insert", RELOCBS.Properties.Resources.UnExpectedErrorAtDAL, ex);
            }
            //return true;
        }
        ...
        public bool Update(CompanyBranchViewModel company, out string result)
        {
            ...
            {
                throw new DataAccessException(Convert.ToString(UserSession.GetUserSession().LoginID), "CompanyBranchDAL", "Update", RELOCBS.Properties.Resources.UnExpectedErrorAtDAL, ex);
            }
        ...
        public bool DeleteById(int id, out string result)
        {
            ...
            try
            {
                using (App_Code.CDALSQL conn = new App_Code.CDALSQL(System.Configuration.ConfigurationManager.ConnectionStrings["RELODB"].ToString()))
                {
                    if (conn.Connect())
                    {
                        ...
                        conn.AddCommand("Comm.CompanyBranchDelete", QueryType.Procedure);
            ...
            {
                throw new DataAccessException(Convert.ToString(UserSession.GetUserSession().LoginID), "CompanyBranchDAL", "DeleteById", RELOCBS.Properties.Resources.UnExpectedErrorAtDAL, ex);
            }
        ...
        public DataTable GetDetailById(int? id, int LoginID)
        {
            ...
            try
            {
                string query = string.Format("exec [Comm].[GETCompanyBranchDetail] @SP_CompBranchID={0},@SP_LoginID={1}",
        ...
        public IEnumerable<CompanyBranch> GetCompanyBrachList(int pPageIndex, int pPageSize, string pOrderBy, int pOrder, int? CountryID, int? CityID, int? CompID,int? pisActive, string SearchKey, int LoggedinUserID, out int TotalCount)
        {
            TotalCount = 0;
            try
            {
                string query = string.Format("exec Comm.GETCompanyBranchForGrid @SP_PageIndex={0},@SP_PageSize={1},@SP_OrderBy={2},@SP_Order={3},@SP_CountryID={4},@SP_CityID={5},@SP_CompID={6},@SP_isActive={7},@SP_SearchString={8},@SP_LoginID={9}",
            ...
            {
                throw new DataAccessException(Convert.ToString(LoggedinUserID), "CompanyBranchDAL", "GetCompanyBrachList", RELOCBS.Properties.Resources.UnExpectedErrorAtDAL, ex);
            }
